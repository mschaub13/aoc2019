...
//var testwire = "R12,L1,U100,D549";

wireSteps = 0;

function right(steps, crossmap) {
  console.log("Right " + steps);
  for (i = 1; i <= steps; i++) {
    xBearing++;
    wireSteps++;
    if (!crossmap.hasOwnProperty(xBearing + "|" + yBearing))
      crossmap[xBearing + "|" + yBearing] = wireSteps;
  }

  return crossmap;
}

function left(steps, crossmap) {
...
function broker(nugget, crossmap) {
  dir = nugget.substring(0, 1);
  steps = nugget.substring(1, nugget.length);
  console.log("NUGGET: " + nugget);
  //  console.log("DIR: " + dir);
  //  console.log("STEPS: " + steps);

  if (dir == "R") {
    crossmap = right(steps, crossmap);
  } else if (dir == "L") {
    crossmap = left(steps, crossmap);
  } else if (dir == "U") {
    crossmap = up(steps, crossmap);
  } else if (dir == "D") {
    crossmap = down(steps, crossmap);
  }
...
// wire 1
crossmap1 = new Object();

var wire = wire1.split(",");

xBearing = 0;
yBearing = 0;
console.log(wire.length);
wireSteps = 0;
for (z = 0; z < wire.length; z++) {
  crossmap1 = broker(wire[z], crossmap1);
}

// wire 2
crossmap2 = new Object();
var wire = wire2.split(",");
...
wireSteps = 0;
for (z = 0; z < wire.length; z++) {
  crossmap2 = broker(wire[z], crossmap2);
}

final = -1;
//console.log(Object.keys(crossmap1));
Object.keys(crossmap1).forEach(val => {
  if (crossmap2.hasOwnProperty(val)) {
//        console.log("HIT: " + val.split("|"));
//        hit = val.split("|");
//        x = Math.abs(parseInt(hit[0]));
//        y = Math.abs(parseInt(hit[1]));
//        distance = x + y;

    dist1 = parseInt(crossmap1[val]);
    dist2 = parseInt(crossmap2[val]);
    distance = +dist1 + +dist2;
    if (final == -1) final = distance;
    else if (distance < final) final = distance;
  }
});
